/**
 * RunPod embeddings client that calls /runsync with budget-aware batching to
 * respect request size limits while minimizing HTTP requests.
 */
...
const DEFAULT_BATCH_CHARS = 250_000;
...
export class RunpodQueueEmbeddings implements Embeddings {
  ...
  constructor(opts: {
     ...
     * - DOCS_MCP_EMBEDDING_BATCH_SIZE (default 16)
     * - DOCS_MCP_EMBEDDING_BATCH_CHARS (default 250000)
    ...
    const envBatch = envInt("DOCS_MCP_EMBEDDING_BATCH_SIZE", DEFAULT_BATCH_SIZE);
    const envChars = envInt("DOCS_MCP_EMBEDDING_BATCH_CHARS", DEFAULT_BATCH_CHARS);
    this.maxTextsPerRequest = opts.maxTextsPerRequest ?? envBatch;
    this.maxCharsPerRequest = opts.maxCharsPerRequest ?? envChars;
  }
  ...
  private *makeBatches(texts: string[]): Generator<string[]> {
    let cur: string[] = [];
    let chars = 0;
    let bytes = 0;
    ...
    for (const t of texts) {
      ...
      const wouldOverflow =
        cur.length > 0 &&
        (cur.length >= this.maxTextsPerRequest ||
          chars + t.length > this.maxCharsPerRequest ||
          bytes + tBytes > this.maxPayloadBytes);
  ...
  private async embed(texts: string[]): Promise<number[][]> {
    ...
    const batches = [...this.makeBatches(texts)];
    const out: number[][] = [];
